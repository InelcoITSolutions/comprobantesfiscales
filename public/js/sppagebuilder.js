...
+ function($) {
    ...
    SPPBCarousel.prototype.to = function(pos) {
        ...
        if (this.sliding) return this.$element.one('slid.sppb.carousel', function() {
                that.to(pos)
            }) // yes, "slid"
    ...
    SPPBCarousel.prototype.slide = function(type, next) {
            ...
            var slidEvent = $.Event('slid.sppb.carousel', {
                    relatedTarget: relatedTarget,
                    direction: direction
                }) // yes, "slid"
...
+ function($) {
    ...
    var SPPBModal = function(element, options) {
        ...
        if (this.options.remote) {
            this.$element.find('.sppb-modal-content').load(this.options.remote, $.proxy(function() {
    ...
    SPPBModal.prototype.show = function(_relatedTarget) {
        ...
        this.backdrop(function() {
            ...
            that.enforceFocus()
            var e = $.Event('shown.sppb.modal', {
                relatedTarget: _relatedTarget
            })
            transition ? that.$element.find('.sppb-modal-dialog') // wait for modal to slide in
    ...
    SPPBModal.prototype.hide = function(e) {
        ...
        this.$element.removeClass('in').attr('aria-hidden', true).off('click.dismiss.sppb.modal')
        $.support.transition && this.$element.hasClass('fade') ? this.$element.one('bsTransitionEnd', $.proxy(this.hideModal, this)).emulateTransitionEnd(300) : this.hideModal()
    }
    SPPBModal.prototype.enforceFocus = function() {
        $(document).off('focusin.sppb.modal') // guard against infinite focus loop
    ...
    SPPBModal.prototype.backdrop = function(callback) {
        ...
        if (this.isShown && this.options.backdrop) {
            ...
            this.$backdrop = $('<div class="sppb-modal-backdrop ' + animate + '" />').appendTo(this.$body)
    ...
    $(document).on('click.sppb.modal.data-api', '[data-toggle="sppb-modal"]', function(e) {
            ...
            var option = $target.data('sppb.modal') ? 'toggle' : $.extend({
...
+ function($) {
    ...
    SPPBTooltip.prototype.enter = function(obj) {
        var self = obj instanceof this.constructor ? obj : $(obj.currentTarget).data('sppb.' + this.type)
        if (!self) {
    ...
    SPPBTooltip.prototype.show = function() {
        ...
        if (this.hasContent() && this.enabled) {
            ...
            $tip.detach().css({
                ...
                left: 0,
                display: 'block'
            }).addClass(placement).data('sppb.' + this.type, this)
    ...
    var sppbflickrPhotoStream = function($el, options) {
        ...
        return $.getJSON(url).done(function(data) {
            for (var i = 0; i < options.count; i = i + 1) {
                ...
                $("<img class='sppb-img-responsive' alt='" + pic.title + "' />").attr("src", pic.media.m).appendTo($el).wrap(options.container || '').wrap(['<a target="_blank" href="' + pic.link + '" title="' + pic.title + '"></a>'].join(''));
            }
        });
    };
